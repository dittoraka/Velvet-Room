...
class post_controller extends Controller
{
    ...
    public function loadpost(){
        ...
        $achievment = DB::table('user_achievment')->where('id_user',session('iduser'))->count();
        $dobel=false;
        $badges = DB::table('user_badges')->get();
        foreach ($badges as $item) {
            if($item->id_badges == $achievment/5 && $item->id_user == session('iduser')&&($achievment%5)==0){
                 $dobel = true;
            }
        }
        if(!$dobel && ($achievment%5)==0){
            DB::table('user_badges')->insert(['id_user'=>session('iduser'),'id_badges'=>$achievment/5]);
        }
        return view('user.home',['post'=>$data,'komen'=>$komen,'user'=>$user,'relasi'=>$friend,'like'=>$data2,'chat'=>$message,'achiev'=>$achievment,'badges'=>$badges]);
    }
    ...
    public function loadpostprofil(){
        ...
        $achievment = DB::table('user_achievment')->where('id_user',session('iduser'))->count();
        return view('user.my-profile-feed',['user'=>$data,'komen'=>$komen,'profil'=>$data1,'achiev'=>$achievment]);
    }
    public function postdiss(Request $request){
        ...
        foreach ($achiev as $key) {
            if($key->id_user == session('iduser') && $key->id_achievment == 3){
                $dobel = true;
            }
        }
        if(!$dobel){
            DB::table('user_achievment')->insert(['id_user'=>session('iduser'),'id_achievment'=>3]);
        }
    ...
    public function ngepost(Request $request){
        ...
        $achiev = DB::table('user_achievment')->get();
        foreach ($achiev as $key) {
            if($key->id_user == session('iduser') && $key->id_achievment == 1){
                $dobel = true;
            }
        }
        if(!$dobel){
            DB::table('user_achievment')->insert(['id_user'=>session('iduser'),'id_achievment'=>1]);
        }
    ...
    public function likee(Request $request){
        ...
        $bol = true;$dobel = false;
        $achiev = DB::table('user_achievment')->get();
        foreach ($achiev as $key) {
            if($key->id_user == session('iduser') && $key->id_achievment == 7){
                $dobel = true;
            }
        }
        if(!$dobel){
            DB::table('user_achievment')->insert(['id_user'=>session('iduser'),'id_achievment'=>7]);
        }
